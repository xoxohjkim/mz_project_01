					...
					<c:if test = "${totalCnt != 0}"> 
						<c:forEach var="b" items="${list}" varStatus="status">
							<tr>
								...
								<td>
									<a href="/board?kind=${b.bgr.key}&id=${b.id}">
										<c:if test="${b.fileList.size() != 0 }">
...
<!-- paging -->
		...
		<div class="board_page">
			    <c:if test="${pageMaker.prev}">
			    	<p><a href="boardList${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></p>
			    </c:if> 
				<ul>
				  ...
				  <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
				    ...
				    <p><a href="boardList${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></p>
				  </c:if>
		</div>
